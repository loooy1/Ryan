...
public interface ITaskStageService
{
    ...
    /// <summary>任务完成时附带 RCS 回调的实际站点，用于分拣台库存落位。</summary>
    event Action<string, string?>? TaskFinished;
...
public class TaskStageService : ITaskStageService
{
    ...
    public event Action<string, string?>? TaskFinished;
    ...
    public void Record(TaskStageChangeModel change)
    {
        ...
        if (raiseFinished) TaskFinished?.Invoke(change.TaskId, change.StationCode);
    }
    ...
    {
        Interlocked.Increment(ref _forceGeneration);
    ...
    public async Task<bool> WaitWcsCompletedAsync(string taskId, TimeSpan? timeout = null)
    {
        ...
        var forceGeneration = Volatile.Read(ref _forceGeneration);
        while (true)
        {
            if (Volatile.Read(ref _forceGeneration) != forceGeneration) return false;
            using (var uow = _uow.Create())
            {
