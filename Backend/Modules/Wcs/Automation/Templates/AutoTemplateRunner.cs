...
public class AutoTemplateRunner : IHostedService
{
    ...
    /// <summary>等待本轮所有任务完成 WCS 收尾；分拣任务包含后续回库完成。</summary>
    private async Task WaitRoundCompletion(List<string> taskIds, IReadOnlyDictionary<string, string> taskDetails, string parentId)
    {
        try
        {
            async Task<bool?> WaitOne(string taskId)
            {
                while (!_halted)
                {
                    try { return await _stage.WaitWcsCompletedAsync(taskId, TimeSpan.FromSeconds(1)); }
                    catch (TimeoutException) { }
                }
                return null;
            }

            var outcomes = await Task.WhenAll(taskIds.Select(WaitOne));
            if (_halted || outcomes.Any(outcome => outcome == null)) return;
            if (outcomes.Any(outcome => outcome == false))
            {
                _log.Add(parentId, "本轮存在 WCS 收尾失败任务，已停止成功完成提示", "#f87171");
                _log.CompleteRound(parentId);
                return;
            }
    ...
    private async Task<(List<string> ids, IReadOnlyDictionary<string, string> details)> RunTemplates(List<AutoTemplateDto> tpls, string roundId)
    {
        ...
        async Task RunOne(AutoTemplateDto tpl, int k, string childId, TaskDispatcher.ExecCtx? chainCtx = null)
        {
            ...
            bool TryClaim(InvItem item)
            {
                var codes = new[] { item.Code, item.CargoCode }
                    .Where(code => !string.IsNullOrWhiteSpace(code))
                    .Select(code => code!)
                    .Distinct(StringComparer.OrdinalIgnoreCase)
                    .ToList();
                if (!_invStore.TryPick(codes)) return false;
                pickedBusy.AddRange(codes);
                ctx.SelectedCodes = codes;
                return true;
            }
            bool first = true;
            try
                    ...
                    try
                    {
                        ...
                        if (step.Kind == AutoStepKinds.PickPallet)
                        {
                            ...
                            lock (poolLock)
                            {
                                ...
                                if (!TryClaim(pick))
                                {
                                    _log.Add(childId, $"步骤 {stepNo} 选托盘冲突：{pick.Code} 已被其它轮次领取，等待下轮重试", "#fbbf24");
                                    return;
                                }
                            }
                        ...
                        else if (step.Kind == AutoStepKinds.PickCargo)
                        {
                            ...
                            lock (poolLock)
                            {
                                ...
                                if (!TryClaim(pick))
                                {
                                    _log.Add(childId, $"步骤 {stepNo} 选货物冲突：{pick.Code} 已被其它轮次领取，等待下轮重试", "#fbbf24");
                                    return;
                                }
                            }
                            ctx.SelectedStartMark = pick.Mark;
                        ...
                        else if (step.Kind == AutoStepKinds.PickLoadedPallet)
                        {
                            ...
                            lock (poolLock)
                            {
                                ...
                                if (!TryClaim(pick))
                                {
                                    _log.Add(childId, $"步骤 {stepNo} 选带货托冲突：{pick.Code} 已被其它轮次领取，等待下轮重试", "#fbbf24");
                                    return;
                                }
                            }
                            ctx.SelectedStartMark = pick.Mark;
                        ...
                        else if (step.Kind == AutoStepKinds.RunTemplate)
                        {
                            ...
                            if (ctx.InventoryClaimCommitted)
                            {
                                foreach (var code in ctx.SelectedCodes)
                                    pickedBusy.RemoveAll(item => string.Equals(item, code, StringComparison.OrdinalIgnoreCase));
                                ctx.SelectedCodes.Clear();
                                ctx.InventoryClaimCommitted = false;
                            }
                            if (tid == null) return;
