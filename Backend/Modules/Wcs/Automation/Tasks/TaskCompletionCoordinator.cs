...
/// 非自动化任务在 FINISHED 后执行终点模块。分拣任务完成后由 WCS 在关联子站点中落库存。
/// 队列为进程内队列，当前不承担重启恢复。
/// </summary>
public sealed class TaskCompletionCoordinator : BackgroundService
{
    ...
    private void OnFinished(string taskId, string? actualStationCode)
    {
        // 所有 RCS FINISHED 回调统一进入 WCS 收尾队列；StationCode 只保留在阶段记录中，
        // 不参与库存落位，分拣台库存由 WCS 自己选择关联的空闲子站点。
        QueueCompletion(taskId);
    }
    ...
    private async Task CompleteReservationAsync(string taskId)
    {
        ...
        try
        {
            ...
            {
                // RCS FINISHED 只表示任务完成。实际分拣台库存由 WCS 根据人工分拣台
                // 关联的空闲子站点自行选择，避免收尾逻辑依赖 RCS 库存查询接口。
